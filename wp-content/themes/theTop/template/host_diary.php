...
function delete_diary(value) {
    $.ajax({
        url: "<?php echo esc_url( home_url( '/diary_delete_api' ) ); ?>", 
        type: "POST",
        data: {
            diary_id: value
        },
...
function edit_diary(value) {
    $(window).attr('location','<?php echo esc_url( home_url( '/host_edit_diary' ) ); ?>?diary_id=' + value);
}

function add_diary(value) {
    $(window).attr('location','<?php echo esc_url( home_url( '/host_edit_diary' ) ); ?>');
}
            ...
            if(data['data'] != [])  {
                ...
                for (let i = 0 + <?=$items_per_page * ($page - 1);?>; i < <?=$page * $items_per_page;?>; i++) {
                     ...
                     htmltext += '<li><div class="num"><span>' + (i + 1)
                         ...
                         + diarys[i]['d_title'] + '</span></div><div class="content"><span>' 
                         + diarys[i]['d_content'] + '</span></div><div class="created"><span>' 
                         + diarys[i]['create_at'] 
                         + '</span></div><div class="actions"><button type="button" class="edit" onclick="edit_diary(' 
                         + diarys[i]['d_id'] +')"></button><button type="button" class="delete" onclick="delete_diary(' 
                         + diarys[i]['d_id'] +')"></button></div></li>';
                }
